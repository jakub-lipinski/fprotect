...
class SilktideConsentManager {
  ...
  getBannerContent() {
    const bannerDescription =
      ...
      "<p>Używamy plików cookies, aby strona działała prawidłowo oraz, za Twoją zgodą, do analityki.</p>";

    // Accept button
    const acceptAllButtonText = this.config.text?.prompt?.acceptAllButtonText || 'Akceptuję';
    ...
    // Reject button
    const rejectNonEssentialButtonText = this.config.text?.prompt?.rejectNonEssentialButtonText || 'Odrzucam';
    ...
    // Preferences button
    const preferencesButtonText = this.config.text?.prompt?.preferencesButtonText || 'Ustawienia';
  ...
  getModalContent() {
    const preferencesTitle =
      this.config.text?.preferences?.title || 'Ustawienia cookies';

    const preferencesDescription =
      this.config.text?.preferences?.description ||
      "<p>Możesz zdecydować, czy zgadzasz się na dodatkowe pliki cookies.</p>";
    ...
    // Save button
    const saveButtonText = this.config.text?.preferences?.saveButtonText || 'Zapisz i zamknij';
    ...
    const creditLink = creditLinkText ? `<a class="stcm-credit-link" href="https://silktide.com/consent-manager" target="_blank" rel="noreferrer"${
          ...
          .map((type) => {
            ...
            <fieldset>
              ...
              type.required ? 'checked disabled' : isChecked ? 'checked' : ''
                  ...
                  <span class="stcm-toggle-off" aria-hidden="true">Wył</span>
                  <span class="stcm-toggle-on" aria-hidden="true">Wł</span>
                </label>
              </div>
            </fieldset>
          `;
